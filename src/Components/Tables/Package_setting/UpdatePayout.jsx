...
const UpdatePayout = () => {
  ...
  const [payOutPackageName, setPayOutPackageName] = useState(
    packageData.payOutPackageName || ""
  );
  const [payOutChargeRange, setPayOutChargeRange] = useState(
    packageData.payOutChargeRange || []
  );
      ...
      const fetchPackageData = async () => {
        try {
          const response = await apiGet(
            `apiAdmin/v1/package/getSinglePayOutPackage/${id}`
          );
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const updatedChargeRange = payOutChargeRange.map(
        ({ _id, ...rest }) => rest
      );
      await apiPost(`apiAdmin/v1/package/updatePayOutPackage/${id}`, {
        payOutPackageName,
